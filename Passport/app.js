...
passport.use(new LocalStrategy(function(email, password, done) {
	...
	if(checkInput(email, 'email') === true && checkInput(password, 'password') === true) {
		
		db.getConnection(function(err, tempCont) {
			if(err) {
				return done(null, false);
			} else {
				tempCont.query("SELECT * FROM STD_USER WHERE email = ? AND pass = ?;", [email, password], function(err, result) {
					if(err) {
...
passport.deserializeUser(function(id, done) {
	...
	db.getConnection(function(err, tempCont) {
		if(err) {
			...
			tempCont.query("SELECT * FROM STD_USER WHERE userID = ?;", [id], function(err, result) {
				if(err) {
					console.log(err);
				} else {
					done(null, result[0]);
